...
try {
    ...
    Write-Host "Installing the pinned TensorFlow/Flower runtime first..."
    & $Python -m pip install -c $Constraints "tensorflow==2.10.0" "flwr[simulation]==1.5.0" "ray==2.6.3"
    if ($LASTEXITCODE -ne 0) {
        throw "Core TensorFlow/Flower runtime installation failed."
    }

    # protobuf 3.19.6 ships a legacy namespace .pth file which can fail during
    # Python startup on some modern Windows environments. Python 3.10 supports
    # implicit namespace packages, so removing this compatibility file is safe.
    $SitePackages = Join-Path $Venv "Lib\site-packages"
    Get-ChildItem -Path $SitePackages -Filter "protobuf-*-nspkg.pth" -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue

    Write-Host "Installing Eiffel and the remaining pinned dependencies..."
    ...
    # Remove the legacy protobuf namespace file again in case pip recreated it
    # while resolving the remaining dependencies.
    Get-ChildItem -Path $SitePackages -Filter "protobuf-*-nspkg.pth" -ErrorAction SilentlyContinue |
        Remove-Item -Force -ErrorAction SilentlyContinue

    Write-Host "Checking the installed dependency graph..."
    & $Python -m pip check
    if ($LASTEXITCODE -ne 0) {
        throw "pip check found missing or incompatible runtime dependencies."
    }
